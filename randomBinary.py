...
class RandomBinary():
    ...
    def getNewOrders(self,pair, price,index):
        ...
        price_ma_change = max(min(price/self.prices.at[index,'ma100']-1,self.vars['price_ma_different']),-self.vars['price_ma_different'])
        thresh = self.vars['low_percentage']+(price_ma_change+self.vars['price_ma_different'])/(2*self.vars['price_ma_different'])*(self.vars['high_percentage']-self.vars['low_percentage'])
        r = random.random()
        if r>thresh:
            self.vars['kpos'] = kpos = 0
        else:
            self.vars['kpos'] = kpos = 1
        # self.vars['kpos'] = kpos = random.randint(0,1)
...
if __name__=='__main__':
    ...
    parser.add_argument('-price_ma_different',type=float,required=False,help='Initial Money in the wallet.',default=0.05)
    parser.add_argument('-high_percentage',type=float,required=False,help='Initial Money in the wallet.',default=0.7)
    parser.add_argument('-low_percentage',type=float,required=False,help='Initial Money in the wallet.',default=0.3)
    ...
    vars = {
        ...
        'price_ma_different':args.price_ma_different,
        'high_percentage':args.high_percentage,
        'low_percentage':args.low_percentage
    }
    strategy = RandomBinary(vars)
    result_df = strategy.back_test(df)
    result_df.to_csv(f'{args.output}',index=False)
    print(f"Number of trades: {len(result_df)}")
    k_dist = result_df['long_k'].value_counts().sort_index()
    k_dist.at[k_dist.index.max()+1]=0
    print(k_dist)
    print("------------------------------")
    k_dist = k_dist.diff(-1).dropna()[1:].astype(int)
    print(f"Long K distributions: {k_dist}")

    k_dist = result_df['short_k'].value_counts().sort_index()
    k_dist.at[k_dist.index.max()+1]=0
    print(k_dist)
    print("------------------------------")
    k_dist = k_dist.diff(-1).dropna()[1:].astype(int)
