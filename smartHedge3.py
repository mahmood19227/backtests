...
class SmartHedge(HedgeMode):
    ...
    def Profit(self,tp_side,index):
        price = self.prices.at[index,'open']
        if price < self.prices.at[index,'fast_ma'] <  self.prices.at[index,'slow_ma'] or price > self.prices.at[index,'fast_ma'] >  self.prices.at[index,'slow_ma']:
            market_type = 'trend'
        else:
            market_type = 'range'
...
if __name__=='__main__':
    ...
    df[f'fast_ma'] = ta.SMA(df['open'],timeperiod=args.fast_ma)
    df[f'slow_ma'] = ta.SMA(df['open'],timeperiod=args.slow_ma)
